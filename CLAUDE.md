...
- **modules/message-utils.js** — Pure message/recipient helpers: `parseRecipient`, `recipientKey`, `mergeRecipients` (template recipients are merged into the window's, never assigned over them — existing entries keep their position, so `to[0]` stays the address the user picked), quoting, subject prefix stripping.
- **modules/address-book.js** — Address-book access behind the *optional* `addressBooks` permission, for `{RECIPIENT_NICKNAME}`. Pure vCard helpers (`nicknameFromVCard`, `emailsFromVCard`) plus `lookupNicknameByEmail`. Never throws: no permission / no contact / no nickname all resolve to `""`.
